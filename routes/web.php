...
Route::post('/applications', fn (Request $request) => redirect()->route('dashboard')
    ->with('status', 'Temporary pass submitted.'))->name('application.store');

Route::get('/applications/{application}', fn (string $application) =>
    view('application.show'))->name('application.show');

Route::view('/report/lost-id', 'report.lost-id')->name('report.lost.id');
Route::post('/report/lost-id', fn (Request $request) => redirect()->route('dashboard')
    ->with('status', 'Lost ID reported.'))->name('report.lost.id.store');
...
Route::post('/guest/applications', fn (Request $request) => redirect()->route('guest.dashboard')
    ->with('status', 'Visitor pass submitted.'))->name('guest.application.store');

Route::get('/guest/applications/{application}', fn (string $application) =>
    view('guest.application-show'))->name('guest.application.show');
...
Route::get('/admin/applications/review/{application}', fn (string $application) =>
    view('admin.applications.review'))->name('admin.applications.review');

Route::put('/admin/applications/review/{application}', function (string $application) {
    return redirect()->route('admin.applications.review', ['application' => $application])
